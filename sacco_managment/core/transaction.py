...
def transaction_lists(request):
    sender_transaction = Transaction.objects.filter(
        sender=request.user, transaction_type="transfer").order_by("-date")
    receiver_transaction = Transaction.objects.filter(
        receiver=request.user, transaction_type="transfer").order_by("-date")

    request_sender_transaction = Transaction.objects.filter(
        sender=request.user, transaction_type="request").order_by("-date")
    request_receiver_transaction = Transaction.objects.filter(
        receiver=request.user, transaction_type="request").order_by("-date")
...
def transaction_detail(request, transaction_id):
    ...
    if request.user not in [transaction.sender, transaction.receiver]:
        messages.error(
            request, "You are not authorized to view this transaction.")
