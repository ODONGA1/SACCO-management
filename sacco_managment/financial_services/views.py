...
def sell_crypto(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                crypto_transaction = form.save(commit=False)
                crypto_transaction.user = request.user
                crypto_transaction.transaction_type = 'SELL'
                
                # Process crypto deduction and fiat credit
                wallet = CryptoWallet.objects.get(user=request.user)
                if getattr(wallet, f"{crypto_transaction.crypto_type.lower()}_balance") < crypto_transaction.amount:
                    messages.error(request, "Insufficient crypto balance")
                    return redirect('sell_crypto')
                ...
                crypto_transaction.save()
                messages.success(request, "Crypto sale successful")
                return redirect('crypto_wallet')
            except Exception as e:
                logger.error(f"Crypto sale error: {str(e)}")
                messages.error(request, "Failed to process crypto sale")
                raise
        else:
            messages.error(request, "Invalid form data")
    else:
    ...
    return render(request, 'financial_services/sell_crypto.html', {'form': form})
...
def process_deposit(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                # Process payment gateway integration here
                messages.success(request, "Deposit processed successfully")
                return redirect('deposit_history')
            except Exception as e:
    ...
    return render(request, 'financial_services/process_deposit.html', {'form': form})
...
def process_withdrawal(request):
    ...
    return render(request, 'financial_services/process_withdrawal.html', {'form': form})
